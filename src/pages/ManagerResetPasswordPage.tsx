...
export default function ManagerResetPasswordPage(){
  ...
  return <main className="min-h-screen bg-sand-50 px-4 py-16 dark:bg-tide-950 sm:px-6"><div className="mx-auto max-w-md"><Link to="/manager" className="text-sm font-semibold text-tide-700 dark:text-gold-300">← Manager login</Link><div className="card-surface mt-8 rounded-3xl p-7 shadow-card sm:p-10"><span className="eyebrow">Account recovery</span><h1 className="mt-3 font-display text-4xl text-tide-900 dark:text-sand-50">Reset manager password</h1><form onSubmit={submit} className="mt-7 space-y-4">
    ...
    <button disabled={busy} className="btn-primary w-full disabled:cursor-not-allowed disabled:opacity-60">{busy&&<Loader2 className="h-4 w-4 animate-spin"/>}{busy?"Resetting…":"Reset password"}</button>{status&&<div className="rounded-xl bg-sand-100 p-4 text-sm dark:bg-tide-900">{status}</div>}</form>{status.includes("successfully")&&<button onClick={()=>navigate("/manager")} className="mt-4 btn-outline w-full">Return to login</button>}</div></div></main>
}
